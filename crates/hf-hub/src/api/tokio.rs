...
impl Api {
    ...
    async fn metadata(&self, url: &str) -> Result<Metadata, ApiError> {
        ...
        let response = if response.status().is_redirection() {
            let location_str = headers.get(LOCATION).unwrap().to_str()?.to_string();
            let resolved_url = if location_str.starts_with('/') || !location_str.contains("://") {
                if let Ok(base) = url::Url::parse(url) {
                    if let Ok(joined) = base.join(&location_str) {
                        joined.to_string()
                    } else {
                        location_str
                    }
