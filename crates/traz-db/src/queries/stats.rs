...
impl Db {
    ...
    pub async fn get_last_event_id(&self) -> Result<Option<i64>> {
        let mut stmt = self.conn.prepare("SELECT id FROM events ORDER BY id DESC LIMIT 1").await?;
        let mut rows = stmt.query(()).await?;
        if let Some(row) = rows.next().await? {
    ...
    pub async fn count_events_after(&self, id: i64) -> Result<usize> {
        let mut rows = self.conn.query("SELECT COUNT(*) FROM events WHERE id > ?1", libsql::params![id]).await?;
        if let Some(row) = rows.next().await? {
